...
function setupEventListeners() {
    const form = document.getElementById('reservaFormElement');
    if (form) {
...
function setupAutocomplete() {
    ...
    if (!nomeInput || !suggestionsContainer) return;
...
function updateCalendar() {
    ...
    // Inicializar grid do calendário se ainda não foi criado
    const calendarGrid = document.getElementById('calendar-grid');
    if (calendarGrid && !calendarGrid.hasChildNodes()) {
...
function gerarRelatorio() {
    ...
    let relatorio = '';
    relatorio += '===========================================================================\n';
    relatorio += '                    RELATÓRIO DE RESERVAS E FINANCEIRO\n';
    relatorio += '                    Cachoeira do Bom Jesus - Temporada 2025/2026\n';
    relatorio += '===========================================================================\n\n';
    relatorio += `Período: ${monthNames[mes]} de ${ano}\n`;
    relatorio += `Data de Geração: ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}\n\n`;
    ...
    const valorSinalTotal = reservasAtivas.reduce((sum, r) => sum + (r.valorSinal || 0), 0);
    const saldoRestante = valorTotal - valorSinalTotal;
    ...
    relatorio += `Valor Total Contratado: R$ ${valorTotal.toFixed(2)}\n`;
    relatorio += `Sinais Recebidos: R$ ${valorSinalTotal.toFixed(2)}\n`;
    relatorio += `Saldo a Receber: R$ ${saldoRestante.toFixed(2)}\n`;
    relatorio += `Taxa de Conversão: ${reservasAtivas.length > 0 ? ((reservasConfirmadas.length / reservasAtivas.length) * 100).toFixed(1) : 0}%\n\n`;
    ...
    ['A6', 'A7', 'A8', 'A9'].forEach(unit => {
        ...
        dadosDisponibilidade[unit].valorTotal = reservasAtivas.filter(r => r.unidade === unit).reduce((sum, r) => sum + (r.valor || 0), 0);
    });
    ...
    ['A6', 'A7', 'A8', 'A9'].forEach(unit => {
        ...
        if (reservasUnit.length === 0) {
            ...
            relatorio += `  Receita Total: R$ ${dadosDisponibilidade[unit].valorTotal.toFixed(2)}\n`;
            relatorio += `  Taxa de Ocupação: ${dadosDisponibilidade[unit].taxaGeral}%\n\n`;
            ...
            reservasUnit.forEach((reserva, index) => {
                ...
                relatorio += `  Check-in: ${formatDate(reserva.checkin)} (14h)\n`;
                relatorio += `  Check-out: ${formatDate(reserva.checkout)} (10h)\n`;
                ...
                if (reserva.valorSinal && reserva.valorSinal > 0) {
                    relatorio += `  Sinal Pago: R$ ${reserva.valorSinal.toFixed(2)}\n`;
                ...
                if (reserva.observacoes) {
                    relatorio += `  Observações: ${reserva.observacoes}\n`;
                }
    ...
    ['A6', 'A7', 'A8', 'A9'].forEach(unit => {
        ...
        linha += ` R${dados.valorTotal.toFixed(0)}\n`;
        relatorio += linha;
        
        relatorio += '  ---------+--------+--------+--------+--------+--------+--------+--------+-------\n';
    });
    
    relatorio += '\n  Legenda: Dias Ocupados/Total de Dias no Mês\n\n';
...
function gerarConfirmacaoReserva(reservaId) {
    ...
    const saldo = reserva.valor - (reserva.valorSinal || 0);
    
    let confirmacao = '';
    confirmacao += '═══════════════════════════════════════\n';
    confirmacao += '       CONFIRMAÇÃO DE RESERVA\n';
    confirmacao += '   Cachoeira do Bom Jesus\n';
    confirmacao += '═══════════════════════════════════════\n\n';
    
    confirmacao += `Olá, ${reserva.nome}!\n\n`;
    confirmacao += `Sua reserva foi confirmada com sucesso! 🎉\n\n`;
    
    confirmacao += '📋 DETALHES DA RESERVA:\n';
    confirmacao += '───────────────────────────────────────\n';
    confirmacao += `🏠 Unidade: ${reserva.unidade}\n`;
    confirmacao += `📅 Check-in: ${formatDate(reserva.checkin)} às 14h\n`;
    confirmacao += `📅 Check-out: ${formatDate(reserva.checkout)} às 10h\n`;
    confirmacao += `⏱️ Duração: ${dias} ${dias === 1 ? 'dia' : 'dias'}\n\n`;
    
    confirmacao += '💰 INFORMAÇÕES FINANCEIRAS:\n';
    confirmacao += '───────────────────────────────────────\n';
    confirmacao += `Valor Total: R$ ${reserva.valor.toFixed(2)}\n`;
    if (reserva.valorSinal && reserva.valorSinal > 0) {
        confirmacao += `Sinal Pago: R$ ${reserva.valorSinal.toFixed(2)}\n`;
        confirmacao += `Saldo Restante: R$ ${saldo.toFixed(2)}\n`;
    }
    if (reserva.formaPagamento) {
        confirmacao += `Forma de Pagamento: ${reserva.formaPagamento}\n`;
    }
    confirmacao += '\n';
    
    confirmacao += '📍 INFORMAÇÕES IMPORTANTES:\n';
    confirmacao += '───────────────────────────────────────\n';
    confirmacao += '• Check-in: 14h\n';
    confirmacao += '• Check-out: 10h\n';
    confirmacao += '• Traga documentos pessoais\n';
    confirmacao += '• Animais: consultar política\n';
    confirmacao += '• Não é permitido festas\n\n';
    
    if (reserva.observacoes) {
        confirmacao += '📝 OBSERVAÇÕES:\n';
        confirmacao += '───────────────────────────────────────\n';
        confirmacao += `${reserva.observacoes}\n\n`;
    }
    
    confirmacao += '───────────────────────────────────────\n';
    confirmacao += 'Dúvidas? Entre em contato conosco!\n';
    confirmacao += 'Cachoeira do Bom Jesus 🏖️\n';
    confirmacao += '═══════════════════════════════════════\n';
    
    mostrarModalConfirmacao(confirmacao, reserva);
}

function mostrarModalConfirmacao(confirmacao, reserva) {
    const modal = document.createElement('div');
    modal.style.cssText = `
        position: fixed;
        top: 0;
    ...
    const title = document.createElement('h2');
    title.textContent = '✅ Confirmação de Reserva';
    title.style.cssText = 'color: #1565c0; margin: 0;';
    ...
    const buttonContainer = document.createElement('div');
    buttonContainer.style.cssText = 'display: flex; gap: 10px; justify-content: flex-end; flex-wrap: wrap;';
    ...
    whatsappBtn.className = 'btn-primary';
    whatsappBtn.style.cssText = 'background: #25D366; color: white;';
    whatsappBtn.onclick = () => enviarWhatsApp(confirmacao, reserva);
    
    const downloadBtn = document.createElement('button');
    downloadBtn.textContent = '💾 Baixar TXT';
    downloadBtn.className = 'btn-secondary';
    downloadBtn.onclick = () => {
        const blob = new Blob([confirmacao], { type: 'text/plain;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `Confirmacao_${reserva.nome.replace(/\s/g, '_')}_${reserva.unidade}.txt`;
        a.click();
        URL.revokeObjectURL(url);
    };
...
function enviarWhatsApp(confirmacao, reserva) {
    let telefone = reserva.whatsapp || '';
    ...
    const mensagem = encodeURIComponent(confirmacao);
...
function showReservaForm() {
    ...
    const suggestions = document.getElementById('nomeSuggestions');
...
// ⚠️ CORREÇÃO CRÍTICA: Adicionar async
...
function checkReservationConflicts(newReserva, editingId = null) {
    ...
    const conflictingReservas = reservas.filter(reserva => 
        reserva.unidade === newReserva.unidade && 
        reserva.status !== 'Canc' && 
        reserva.id !== editingId
    );
        ...
        row.innerHTML = `
            ...
            <td>
                <div style="display: flex; gap: 5px; flex-wrap: wrap;">
                    <button class="btn-secondary" onclick="gerarConfirmacaoReserva('${reserva.id}')" 
                            style="background: #4caf50; color: white; padding: 4px 8px; font-size: 0.75rem; white-space: nowrap;">
                        ✅ Confirmar
                    </button>
                    <button class="btn-secondary" onclick="editReserva('${reserva.id}')" 
                            style="padding: 4px 8px; font-size: 0.75rem;">
                        ✏️ Editar
                    </button>
                    <button class="btn-secondary" onclick="deleteReserva('${reserva.id}')" 
                            style="background: #f44336; color: white; padding: 4px 8px; font-size: 0.75rem;">
                        🗑️ Excluir
                    </button>
                </div>
            </td>
...
function updateStats() {
    ...
    const confEl = document.getElementById('confirmadas');
    const resvEl = document.getElementById('reservadas');
    const execEl = document.getElementById('executadas');
    
    if (totalEl) totalEl.textContent = total;
    if (confEl) confEl.textContent = confirmadas;
    if (resvEl) resvEl.textContent = reservadas;
    if (execEl) execEl.textContent = executadas;
}
...
function importContatos() {
    ...
    if (!text) { 
        alert('Cole a lista de contatos antes de importar.'); 
        return; 
    }
...
function updateContatosStats() {
    ...
    const whatsEl = document.getElementById('com-whatsapp');
    const aguardEl = document.getElementById('aguardando');
    
    if (totalEl) totalEl.textContent = total;
    if (whatsEl) whatsEl.textContent = comWhatsapp;
    if (aguardEl) aguardEl.textContent = aguardando;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const checkFirebase = setInterval(() => {
        if (window.firebaseDb && typeof window.initializeApp === "function") {
            clearInterval(checkFirebase);
            console.log('🔥 Firebase pronto, inicializando app...');
    ...
    setTimeout(() => {
        ...
        if (!window.firebaseDb) {
            console.error('❌ Timeout: Firebase não inicializou em 10s');
            alert('Erro ao conectar com o Firebase. Por favor, recarregue a página.');
        }
    }, 10000);
});